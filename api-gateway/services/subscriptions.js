...
module.exports.update_subscription = (reqHeaders, subs_id, type, state, inserted_at) => {
    return axios.put(
        this.subscriptionsRoute('/subscriptions/' + subs_id),
        { "type": type, "state": state, "inserted_at": inserted_at },
        createHeaders(reqHeaders)
    ).then((result) => {
        let resp = result.data
        if (resp != null) {
...
module.exports.save_payment = (reqHeaders, subs_id, extra) => {
    return axios.post(
        this.subscriptionsRoute('/subscriptions/' + subs_id + '/payments'),
        { "extra": extra },
        createHeaders(reqHeaders)
