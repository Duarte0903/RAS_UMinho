...
def message_queue_setup(channel):
    ...
    channel.queue_declare(queue="rotation-requests")
    channel.queue_bind(
...
def publish_request_message(channel, routing_key, request_id, procedure, parameters):
    ...
    channel.basic_publish(
        exchange="picturas.tools",
        routing_key=routing_key,  # Use key configured to route to the rotation tool queue (test purposes)
    ...
    try:
        while True:
            for file_name in os.listdir(PICTURAS_SRC_FOLDER):
                ...
                publish_request_message(channel, "requests.rotation", request_id, "rotation", rotation_parameters)
